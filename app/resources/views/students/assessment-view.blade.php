...
@section('content')
  @php
    $savedAnswers = $submission->responses->pluck('answer', 'question_id')->toArray();
  @endphp
              ...
              <div class="card-body">
                ...
                @foreach($section->questions as $question)
                  <div class="mb-4">
                    ...
                    @php
                      $savedAnswersRadio = $submission->responses->pluck('option_id', 'question_id')->toArray();                    
                    @endphp
                        ...
                        <div class="option-box">
                          <div class="form-check">
                            <input class="form-check-input" type="radio" name="answers[{{$question->id}}]" value="{{$option->id}}"
                              ...
                              {{ isset($savedAnswersRadio[$question->id]) && $savedAnswersRadio[$question->id] == $option->id ? 'checked' : '' }}
                            >
                  ...
                  @if($question->type == 'checkbox')
                    @php
                      $savedAnswers = $submission->responses->mapWithKeys(function ($response) {
                          $value = $response->answer;

                          return [
                              $response->question_id => is_string($value) && str_starts_with($value, '[')
                        ...
                        <div class="option-box">
                          <div class="form-check">
                            <input
                                ...
                                name="answers[{{$question->id}}][]"
                                value="{{$option->id}}"
                                {{ in_array($option->id, $savedAnswers[$question->id] ?? []) ? 'checked' : '' }}
                            >
  ...
  <script>
    ...
    $('#assessmentForm').submit(function (e) {
      ...
      $.ajax({
        ...
        data: formData,
        success: function () {
          window.location.href = "{{ route('startForm', $form->id) }}";
        }
      });
    });
  ...
  <script>
    $(document).ready(function () {
      ...
      function validateCurrentQuestion() {
        let currentDiv = questions.eq(currentQ);
        let inputs = currentDiv.find('input, textarea');
        let isValid = true;
        inputs.each(function () {
          if (!this.checkValidity()) {
            this.reportValidity();
